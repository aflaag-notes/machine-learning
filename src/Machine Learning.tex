...
\begin{document}
    ...
    \begin{frameddefn}{True error}
        Given an ML problem, the \tbf{true error} of a hypothesis $h$ --- with respect to the target function $f$ of the problem, and to the distribution $\mathcal D$ --- is the probability that $h$ will \tit{misclassify} an instance drawn at random according to $\mathcal D$: $$\mathrm{error}_\mathcal D (h) := \Pr_{x \in \mathcal D}{(f(x) \neq h(x))}$$
    \end{frameddefn}
    ...
    \begin{frameddefn}{Accuracy}
        Given an ML problem, the \tbf{accuracy} of a hypothesis $h$ is defined as follows: $$\mathrm{accuracy}(h) := 1 - \mathrm{error}(h)$$
    \end{frameddefn}
    ...
    \begin{frameddefn}{True comparison}
        Given two hypotheses $h$ and $h'$ of some ML problem, the \tbf{true comparison} is defined as $$d := \mathrm{error}_\mathcal D(h) - \mathrm{error}_\mathcal D(h')$$ and its \tit{estimator} is defined as $$\hat d := \mathrm{error}_{S}(h) - \mathrm{error}_{S'}(h')$$
    \end{frameddefn}
    ...
    \begin{frameddefn}{Overfitting}
        Consider an hypothesis $h \in H$ of an ML problem computed over some training data; $h$ \tbf{overfits} the training data if there exists an alternative hypothesis $h' \in H$ such that $$\mathrm{error}_S(h) < \mathrm{error}_S(h')$$ but $$\mathrm{error}_\mathcal D(h) > \mathrm{error}_\mathcal D(h')$$
    \end{frameddefn}
    ...
    \begin{framedalgo}{$k$-fold Cross Validation (hypotheses)}
        Given an ML problem, the algorithm returns an \tit{unbiased estimator} for the true error. \\
        \hrule
        \quad
        \label{alg:k-fold1}
        \begin{algorithmic}[1]
    ...
    The problem arises from having \tbf{too many} \tit{negative samples} in the dataset. This poses a \tit{significant challenge} in ML because an \tit{overabundance} of samples of the same type makes it difficult to develop models that can classify accurately.
