...
\begin{document}
    ...
    There are multiple types of Machine Learning (ML) problems, such as \tit{dataset type} and \tit{target function type}, some of which include:

    \begin{itemize}
        \item \tbf{classification}, which involves the \tit{classification} of inputs into predetermined categories;
        \item \tbf{regression}, which involves the \tit{approximation} of functions defined over $\R$;
        \item \tbf{unsupervised learing}, a type of ML problem where the model learns patterns from \tit{unlabeled data};
        \item \tbf{reinforcement learning}, in which an agent learns to make decisions by interacting with an \tit{environment} and receiving \tit{rewards} or \tit{penalties} based on its actions.
    \end{itemize}
    ...
    \begin{frameddefn}{Hypothesis consistency}
        Given an ML problem defined by a target function $\func{c}{X}{Y}$ --- for some sets $X$ and $Y$ --- and a training dataset $D = \{(x, c(x))\}$, a hypothesis $h \in H$ is said to be \tbf{consistent with $D$} if and only if $$\forall x \in D \quad h(x) = c(x)$$
    \end{frameddefn}

    Note that this definition is important, because $h(x)$ can be evaluated for any $x \in X$, but only inputs that appear in the dataset can be verified, for which $c(x)$ is known. Therefore, \tit{consistency} should be desirable for a hypothesis, since the real goal of an ML system is to find \tit{the best} $h$ that predicts correct values of $h(x')$, for instances $x' \notin X_D$, with respect to the unknown values $c(x')$.
    ...
    Additionally, consider a hypothesis $h$, solution of a learning problem obtained from $S$. What is the \tit{best estimate} of the accuracy of $h$, over future instances drawn from $\mathcal D$? What is the \tit{probable error} in this accuracy estimate?

    \begin{frameddefn}{True error}
        Given a classification problem, the \tbf{true error} of a hypothesis $h$ --- with respect to the target function $f$ of the problem, and to the distribution $\mathcal D$ --- is the probability that $h$ will \tit{misclassify} an instance drawn at random according to $\mathcal D$: $$\mathrm{error}_\mathcal D (h) := \Pr_{x \in \mathcal D}{(f(x) \neq h(x))}$$
    \end{frameddefn}

    Note that the \tbf{true error} of a classifcation problem \tit{does not} refer to samples in the dataset, but to \tit{any sample} in the input space, extracted accordin to $\mathcal D$. Moreover, note that this definition is \tit{not operational}, and this term \tbf{cannot be computed}, because $f(x)$ \tit{is not known}. In fact, this is a \tbf{target error}. Instead, consider the following type of error.

    \begin{frameddefn}{Sample error}
        Given a classification problem, the \tbf{sample error} of a hypothesis $h$ --- with respect to the target function $f$ of the problem, and to the data sample $S$ --- is the proportion of examples that $h$ misclassifies: $$\mathrm{error}_S(h) := \dfrac{1}{n} \sum_{x \in S}{\delta(f(x), h(x))}$$ where $\delta$ is an indicator variable such that $$\delta(f(x), h(x)) := \soe{ll}{1 & f(x) \neq h(x) \\ 0 & f(x) = h(x)}$$
    \end{frameddefn}
    ...
    \begin{frameddefn}{Accuracy}
        Given a classification problem, the \tbf{accuracy} of a hypothesis $h$ is defined as follows: $$\mathrm{accuracy}(h) := 1 - \mathrm{error}(h)$$
    \end{frameddefn}
    ...
    \begin{example}[Confidence intervals]
        ...
        Note that \tit{increasing} $n$ \tbf{reduces the variance}, therefore it will me more probable that the \tit{true error} will fall within an interval.
    \end{example}
    ...
    \begin{framedalgo}[label={unbiased_estimator}]{Unbiased estimator}
        Given a classification problem, the algorithm returns an unbiased estimator for the \tit{true error}. \\
        \hrule
        \quad
        \label{unbiased_estimator}
        \begin{algorithmic}[1]
            \Function{unbiasedEstimator}{$D$}
    ...
    The split performed in the first line of the algorithm \tit{partitions} the input dataset $D$ into two sets $T$ and $S$ --- therefore, $T \cup S = D$ and $T \cap S = \varnothing$. The set $T$ is typically referred to as \textbf{training set}, and since $h$ is computed based on $T$, the algorithm \tit{guarantees} that $h$ is \tbf{independent} of $S$, the set used to assess the \tit{sample error}. Note that $\mathrm{error}_S(h) := \frac{1}{n} \sum_{x \in S}{\delta(f(x), h(x))}$ can be computed because $S$ comes from $D$, therefore $f(x)$ is known for each $x \in S$. Note that it is \tit{crucial} that $D$ is partitioned into $T$ and $S$ \tbf{randomly}, as any split based on specific criteria would introduce \tit{bias} into the partition.

    Moreover, note that the $\frac{2}{3}$ split is a \tit{good trade-off} between $\abs T$ and $\abs S$, because:

    \begin{itemize}
        \item having more samples from \tit{training} and less for \tit{testing} improves the performance of the model, but $\mathrm{error}_S(h)$ does not approximate well $\mathrm{error}_\mathcal D (h)$;
        \item having more sample from \tit{testing} and less for \tit{training} reduces the variance of estimation, and $\mathrm{error}_S(h)$ approximates $\mathrm{error}_\mathcal D (h)$ well, but its value mey be unsatisfactory.
    \end{itemize}

    Additionally, in general computing $\mathrm{error}_S(h)$ is \tit{insufficient}, as it represents just one realization of the random variable. To estimate $E[\mathrm{error}_S(h)]$, it can be approximated by \tit{averaging} the computed values of $\mathrm{error}{S_i}(h)$ over several subsets $S_i$.

    To compare two hypothesis, the following definition can be utilized.

    \begin{frameddefn}{True comparison}
        Given two hypotheses $h$ and $h'$ of some classification problem, the \tbf{true comparison} is defined as $$d := \mathrm{error}_\mathcal D(h) - \mathrm{error}_\mathcal D(h')$$ and its \tit{estimator} is defined as $$\hat d := \mathrm{error}_{S}(h) - \mathrm{error}_{S'}(h')$$
    \end{frameddefn}

    Note that, in this definition, $\hat d$ is an \tbf{unbiased estimator} for $d$ it and only if $h, h', S$ and $S'$ are independent of each other, but it is \tit{still valid} if $S = S'$. Lastly, note that $d = E[\hat d]$.
    
    \begin{frameddefn}{Overfitting}
        Consider an hypothesis $h \in H$ of a classification problem computed over some training data; $h$ \tbf{overfits} the training data if there exists an alternative hypothesis $h' \in H$ such that $$\mathrm{error}_S(h) < \mathrm{error}_S(h')$$ but $$\mathrm{error}_\mathcal D(h) > \mathrm{error}_\mathcal D(h')$$
    \end{frameddefn}
    ...
    \begin{framedalgo}[label={k-fold1}]{$k$-fold Cross Validation (hypotheses)}
        Given a classification problem, the algorithm returns an \tit{unbiased estimator} of the \tit{expected value} of the the \tit{sample error}. \\
        \hrule
        \quad
        \begin{algorithmic}[1]
            \Function{kFoldCrossValidationHyps}{$D$, $L$, $k$, $\alpha$}
                \State Partition $D$ into $k$ disjoint sets $S_1, \ldots , S_k$, such that $\forall i \in [i, k] \quad \abs{S_i} > \alpha$
                \For{$i \in [1, k]$}
                    \State $T_i := D - S_i$ \Comment{note that $T_i \cap S_i = \varnothing$}
                    \State $h_i := L(T_i)$
    ...
    \begin{framedalgo}[label={k-fold2}]{$k$-fold Cross Validation (learning algorithms)}
        ...
        \quad
        \begin{algorithmic}[1]
            \Function{kFoldCrossValidationAlgs}{$D$, $L_A$, $L_B$, $k$, $\alpha$}
                \State Partition $D$ into $k$ disjoint sets $S_1, \ldots , S_k$, such that $\forall i \in [i, k] \quad \abs{S_i} > \alpha$
                \For{$i \in [1, k]$}
                    \State $T_i := D - S_i$ \Comment{note that $T_i \cap S_i = \varnothing$}
                    \State $h_A := L_A(T_i)$
    ...
    Note that if $\bar \delta < 0$, then $L_A$ is estimated to be \tit{better than} $L_B$.

    \subsection{Classification accuracy}

    In \tit{classification tasks}, the terms \tbf{true positive}, \tbf{false negative}, and \tbf{false positive} and \tbf{true negative} refer to the various outcomes of a binary classification model. The effectiveness of a model is assessed based on how \tit{accurately} it classifies instances into these categories, which ultimately reflects its overall performance. The following table illustrates the various terms in detail.
    ...
    \begin{frameddefn}{Error rate and accuracy}
        The \tbf{error rate} of classification problems is defined as follows: $$\mathrm{error \ rate} := \dfrac{\abs{\mathrm{errors}}}{\abs{\mathrm{instances}}} = \dfrac{\mathrm{FN} + \mathrm{FP}}{\mathrm{TP} + \mathrm{TN} + \mathrm{FP} + \mathrm{FN}}$$ Additionally, \tbf{accuracy} is defined as $$\mathrm{accuracy} := 1 - \mathrm{error \ rate} = \dfrac{\mathrm{TP} + \mathrm{TN}}{\mathrm{TP} + \mathrm{TN} + \mathrm{FP} + \mathrm{FN}}$$
    \end{frameddefn}

    Accuracy can be used as a performance metric for classification problems, but is it always \tit{suitable}? For instance, consider the following CL problem, where

    \begin{itemize}
        \item the target function has the form $\func{f}{X}{\{+, -\}}$
        \item the dataset $D$ contains 90\% of \tit{negative samples}
    \end{itemize}
    
    and consider two hypotheses $h_1$ and $h_2$ such that $h_1$ has 90\% of \tit{accuracy} and $h_2$ has 85\% of \tit{accuracy} --- assuming that both hypotheses were computed as accurately as possible, without bias. This may suggest that $h_1$ is better than $h_2$, but consider the following hypothesis: $$h_1 \in H \mid \forall x \in X \quad h(x) = -$$ clearly, this hypothesis has 90\% accuracy because, when tested on $D$, which consists of 90\% \tit{negative samples}, it will be correct exactly 90\% of the time. However, this hypothesis is not a \tit{suitable solution} for a CL problem, as it fails to model any realistic scenarios.

    The issue arises from having \tbf{too many} \tit{negative samples} in the dataset. This poses a \tit{significant challenge} in ML because an \tit{overabundance} of samples of the same type makes it difficult to develop models that can classify accurately.

    This example clearly shows that, in general, \tbf{accuracy} alone is \tit{not enough} to assess the performance of the solution of a classification problem, as \tbf{unbalanced datases} are \tit{very common} in real-world scenarios (e.g. anomaly detection).

    Now, consider the following metrics.

    \begin{frameddefn}{Recall and precision}
        \tbf{Recall} and \tbf{precision} are defined as follows: $$\mathrm{recall} := \dfrac{\abs{\mathrm{true \ positives}}}{\abs{\mathrm{real \ positives}}} = \dfrac{\mathrm{TP}}{\mathrm{TP} + \mathrm{FN}}$$ $$\mathrm{precision} := \dfrac{\abs{\mathrm{true \ positives}}}{\abs{\mathrm{predicted \ prositives}}} = \dfrac{\mathrm{TP}}{\mathrm{TP} + \mathrm{FP}}$$
    \end{frameddefn}

    These metrics are generally \tbf{better suited} for classification problems and are widely adopted across various branches of \tit{statistics}. In particular, \tbf{recall} measures how good is the system at \tit{avoiding false negatives} --- indeed, recall $= 0$ when FN $= 0$ --- while the \tbf{precision} measures the system's ability to \tit{avoid false positives} --- in fact, precision $= 0$ when FP $= 0$.

    The significance of \tit{false positives} and \tit{false negatives} varies depending on the system in question. For example, consider an autonomous car that must learn to identify whether a pedestrian is present in front of it. In this scenario, \tit{false positives} --- instances where the car mistakenly detects a pedestrian --- are far less critical than \tit{false negatives}, where the car fails to recognize an actual pedestrian. This means that, in assessing the performance of such a classification system, \tbf{recall} is much more relevant.

    \begin{frameddefn}{$F1$-score}
        The \tbf{$F1$-score} is defined as follows: $$F1\mathrm{-score} := 2 \cdot \dfrac{\mathrm{precision} \cdot \mathrm{recall}}{\mathrm{precision} + \mathrm{recall}}$$
    \end{frameddefn}

    This score measures the trade-off between \tit{precision} and \tit{recall}. Note that there are are types of \tit{performance measures}, depending on the context, such as
    
    \begin{itemize}
        \item \tbf{true positive rate}: $$\mathrm{TPR} := \dfrac{\mathrm{TP}}{\mathrm{TP} + \mathrm{FN}}$$
        \item \tbf{true negative rate}: $$\mathrm{TNR} := \dfrac{\mathrm{TN}}{\mathrm{TN} + \mathrm{FP}}$$
        \item \tbf{false positive rate}: $$\mathrm{FPR} := \dfrac{\mathrm{FP}}{\mathrm{TN} + \mathrm{FP}}$$
        \item \tbf{false negative rate}: $$\mathrm{FNR} := \dfrac{\mathrm{FN}}{\mathrm{TP} + \mathrm{FN}}$$
    \end{itemize}

    Note that \cref{fpfnfptn} can be extended with other type of \tbf{classes} too --- in the table shown, there are two classes, namely the \tit{true class} and the \tit{predicted class}. For instance, if there are $n$ type of classes, the following table can be created
    ...
    This matrix is called \tbf{multi-class confusion matrix}. Note that a cell $c_{i, j}$ represents how many times an instance of class $C_i$ is classified in class $C_j$. Moreover, the \tbf{diagonal} of this matrix contains the \tit{accuracy} of each class, and the values that lie outside the diagonal can be used to determine which classes are \tbf{confused} the most. Clearly, the sum $\sum_{j = 1}^n {c_{i, j}}$ of the cells in the $i$-th row add up to the total number of saples of class $C_i$ of the dataset. Usually, confusion matrices are represented as colored tables.

    \centeredimage[A multi-class confusion matrix.]{0.25}{../assets/confusion.png}

    \subsection{Regression performance}

    Consider a \href{https://en.wikipedia.org/wiki/Regression_analysis}{regression problem} defined by a target function $\func{f}{X}{\R^d}$ and a testing set $S = \{(x_i, t_i) \mid i \in [1, n], x_i \in X, t_i \in \R^d\}$. The \tbf{performance} of regression problems can be assessed using multiple methods, which are all expressed in terms of $$|\hat f(x_i) - t_i|$$ for some $(x_i, t_i) \in S$, which measures how much the estimated value $\hat f(x_i)$ of $x_i$ differs from the known value $t_i$ of $x_i$.

    \begin{frameddefn}{Mean Absolute Error}
        Given a regression problem, the \tbf{mean absolute error} (MAE) is defined as follows $$\mathrm{MAE} := \dfrac{1}{n} \sum_{i = 1}^n {|\hat f(x_i) - t_i|}$$
    \end{frameddefn}

    \begin{frameddefn}{Mean Squared Error}
        Given a regression problem, the \tbf{mean squared error} (MSE) is defined as follows $$\mathrm{MSE} := \dfrac{1}{n} \sum_{i = 1}^n {\rbk{\hat f(x_i) - t_i}^2}$$ Additionally, the \tbf{root mean squared error} (RMSE) is defined as $\sqrt{\mathrm{MSE}}$.
    \end{frameddefn}

    \begin{frameddefn}{Mean Absolute Percentage Error}
        Given a regression problem, the \tbf{mean absolute percentage error} (MAPE) is defined as follows $$\mathrm{MAPE} := \dfrac{1}{n} \sum_{i = 1}^n{\dfrac{|\hat f(x_i) - t_i|}{t_i}}$$
    \end{frameddefn}

    \begin{frameddefn}{Mean Squared Percentage Error}
        Given a regression problem, the \tbf{mean absolute percentage error} (MAPE) is defined as follows $$\mathrm{MAPE} := \dfrac{100}{n} \sum_{i = 1}^n{\dfrac{|\hat f(x_i) - t_i|}{t_i}}$$
    \end{frameddefn}

    \begin{frameddefn}{Mean Squared Percentage Error}
        Given a regression problem, the \tbf{mean absolute percentage error} (MAPE) is defined as follows $$\mathrm{MAPE} := \dfrac{100}{n} \sum_{i = 1}^n{\rbk{\dfrac{\hat f(x_i) - t_i}{t_i}}^2}$$ Analogously, the \tbf{root mean squared percentage error} (RMSPE) is defined as $\sqrt{MSPE}$.
    \end{frameddefn}
    ...
    \begin{framedalgo}[label={k-fold3}]{$k$-fold Cross Validation (regression)}
        Given a regression problem, the algorithm returns an \tit{unbiased estimator} of the \tit{expected value} of the the \tit{sample error}. \\
        \hrule
        \quad
        \begin{algorithmic}[1]
            \Function{kFoldCrossValidationRegr}{$D$, $L$, $k$, $\alpha$}
                \State Partition $D$ into $k$ disjoint sets $S_1, \ldots , S_k$, such that $\forall i \in [i, k] \quad \abs{S_i} > \alpha$
                \For{$i \in [1, k]$}
                    \State $T_i := D - S_i$ \Comment{note that $T_i \cap S_i = \varnothing$}
                    \State $h_i := L(T_i)$
